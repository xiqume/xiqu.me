...
function sys_ShowListPage($num,$pagenum,$dolink,$dotype,$page,$lencord,$ok,$search="",$add){
	...
	if($pagenum<>1)
	{
		$pagetop="<li><a href='".$dolink.$add['dofile'].$dotype."'>".$fun_r['startpage']."</a></li>";
	}
	else
	{
		$pagetop="<li><span>".$fun_r['startpage']."</span></li>";
	}
	//上一页
	if($pagenum<>1)
	{
		...
		$pagepri="<li><a href='".$dolink.$prido."'>".$fun_r['pripage']."</a></li>";
	}
	else
	{
		$pagepri="<li><span>".$fun_r['pripage']."</span></li>";
	}
	//下一页
	if($pagenum<>$page)
	{
		...
		$pagenext="<li><a href='".$nextpagelink."'>".$fun_r['nextpage']."</a></li>";
	}
	else
	{
		$pagenext="<li><span>".$fun_r['nextpage']."</span></li>";
	}
	//尾页
	if($pagenum==$page)
	{
		$pageeof="<li><span>".$fun_r['lastpage']."</span></li>";
	}
	else
	{
		$lastpagelink=$repagenum&&$repagenum<$page?eReturnRewritePageLink2($add,$page):$dolink.$add['dofile'].'_'.$page.$dotype;
		$pageeof="<li><a href='".$lastpagelink."'>".$fun_r['lastpage']."</a></li>";
	}
	$options="";
	$pagelist="";
	//取得下拉页码
	if(empty($search))
	{
		for($go=1;$go<=$page;$go++)
		{
			...
			$myoptions.= '<li><a href="'.$thispagelink.'">'.$go.'</a></li>';
		}
	}
	else
	{
		...
		$pagelist.=str_replace('<li><a href="'.$dolink.$add['dofile'].'_'.$ok.$dotype.'">','<li class="cur-page"><a href="'.$dolink.$add['dofile'].'_'.$ok.$dotype.'">',$search);;
	}
	$options="<select name=select onchange=\"self.location.href=this.options[this.selectedIndex].value\">".$options."</select>";
	//分页
	$pagelink=$pagetop.$pagepri.$pagelist.$pagenext.$pageeof;
	//替换模板变量
